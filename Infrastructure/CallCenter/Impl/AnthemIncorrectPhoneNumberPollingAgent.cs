...
namespace Falcon.App.Infrastructure.CallCenter.Impl
{
    ...
    public class AnthemIncorrectPhoneNumberPollingAgent : IAnthemIncorrectPhoneNumberPollingAgent
    {
        ...
        public AnthemIncorrectPhoneNumberPollingAgent(ILogManager logManager, ISettings settings, IHealthPlanCustomerExportService healthPlanCustomerExportService, ICorporateTagRepository corporateTagRepository,
            IUniqueItemRepository<CorporateAccount> corporateAccountReporsitory)
        {
            ...
            _logger = logManager.GetLogger("AntheIncorrectPhoneNumberExort");
            ...
            try
            {
                ...
                foreach (var customTag in _settings.AnthemCustomTags)
                {
                    try
                    {
                        ...
                        destinationPath = Path.Combine(destinationPath, "IncorrectPhone");

                        var fileName = string.Format("HLTHFAIR_IncorrectPhone_{0}.csv", DateTime.Today.ToString("yyyyMMdd"));
                        _healthPlanCustomerExportService.AnthemIncorrectPhoneNumbreCustomerExport(filter, destinationPath, fileName, _logger);

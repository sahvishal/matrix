...
{
    public class BioSound2HcpCarotidParser : ITestParser
    {
        ...
        private const string ConstforEdv = "ed vel";
        private const string ConstforPsv = "ps vel";

        public BioSound2HcpCarotidParser(string pathToReportXmlFile, string mediaLocation, ILogger logger)
        {
        ...
        public HcpCarotidTestResult MapXmlToDomainObject(string filePathtoXml)
        {
            ...
            testResult.ResultImages = _mediaHelper.GetMediaSortedByDecimalValue(Directory.GetParent(filePathtoXml).FullName, _mediaLocation, TestType.HCPCarotid.ToString(), out isExceptionCaused).ToList();
        ...
        public HcpCarotidTestReadings GetHcpCarotidTestReadings(XDocument xDoc, string sideToCheck)
        {
            ...
            {
                var readings = new HcpCarotidTestReadings
                {
            ...
            try
            {
                ...
                var subMeasures = (from q in query
                                   where q.Attribute("Description") != null
                                         && q.Attribute("Description").Value.ToLower().Contains(sideToCheck + " " + ConstforIca)

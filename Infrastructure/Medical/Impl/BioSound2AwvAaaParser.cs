...
{
    public class BioSound2AwvAaaParser : ITestParser
    {
        private ILogger _logger;
        private string _errorSummary;

        private readonly Service.TestResultService _testResultService;
        private List<ResultReading<int>> _awvAaaReadings;
        ...
        public BioSound2AwvAaaParser(string pathToReportXmlFile, string mediaLocation, ILogger logger, long eventId, long customerId)
        {
        ...
        public AwvAaaTestResult MapXmlToDomainObject(string filePathtoXml)
        {
            ...
            testResult.ResultImages = _mediaHelper.GetMediaSortedByDecimalValue(Directory.GetParent(filePathtoXml).FullName, _mediaLocation, TestType.AwvAAA.ToString(), out isExceptionCaused).ToList();
        ...
        private void GetAwvAaaSaggitalView(AwvAaaTestResult testResult, XDocument xDoc)
        {
            try
            {
                ...
                if (aoProxSag == null && aoMidSag == null && aoDistSag == null) return;

                IEnumerable<string> viewLocs;
                var aortaValue = GetMaximumOfThree(aoProxSag ?? 0, aoMidSag ?? 0, aoDistSag ?? 0, out viewLocs);

                if (aortaValue == null) return;

                if (_awvAaaReadings.Any(r => r.Label == ReadingLabels.AortaSize))
                {
                ...
                if (viewLocs != null && viewLocs.Any() && _awvAaaReadings.Any(r => r.Label == ReadingLabels.AortaRangeSaggitalView))
                {
                    var sagViewFindings = _testResultService.GetAllStandardFindings<int>((int)TestType.AwvAAA, (int)ReadingLabels.AortaRangeSaggitalView);

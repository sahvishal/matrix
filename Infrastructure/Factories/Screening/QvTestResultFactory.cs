...
{
    public class QvTestResultFactory : TestResultFactory
    {
        private readonly string _toCheckFor = "";

        public QvTestResultFactory(bool isMale)
        {
        ...
        private CompoundResultReading<T> CreateTestReadingforaIntValue<T>(CustomerEventReadingEntity testReadingEntity, int readingId, List<StandardFindingTestReadingEntity> standardFindingTestReadingEntities, T value)
        {
            ...
            var allFindings = new TestResultService().GetAllStandardFindings<T>((int)TestType.Qv, readingId);

            if (testReadingEntity.Value != null && int.TryParse(testReadingEntity.Value, out testReadingValue) == true)
            {
                var findings = (new TestResultService()).GetMultipleCalculatedStandardFinding(testReadingValue, (int)TestType.Qv, readingId);
            ...
            List<StandardFindingTestReadingEntity> standardFindingTestReadingEntities, decimal? value)
        {
            ...
            if (testReading.Finding != null)
                testReading.Finding = new TestResultService().GetAllStandardFindings<decimal?>((int)TestType.Qv, readingId).Find(standardFinding => standardFinding.Id == testReading.Finding.Id);
        ...
        public override TestResult CreateActualTestResult(CustomerEventScreeningTestsEntity customerEventScreeningTestsEntity)
        {
            ...
            var testResult = new QvTestResult(customerEventScreeningTestsEntity.CustomerEventScreeningTestId);
        ...
        {
            var qvTestResult = testResult as QvTestResult;
            var customerEventScreeningTestsEntity = new CustomerEventScreeningTestsEntity(testResult.Id) { TestId = (int)TestType.Qv };

...
{
    public class AwvEkgIppeTestRepository : TestResultRepository
    {
        ...
        public AwvEkgIppeTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public AwvEkgIppeTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new AwvEkgIppeTestResultFactory();
            _testResultSynchronizationService = new AwvEkgIppeTestResultSynchronizationService(callingSource);
            _callingSource = callingSource;
        }

        public AwvEkgIppeTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.AwvEkgIPPE);
            var testResult = _testResultFactory.CreateTestResults(customerEventScreeningTests).SingleOrDefault();
            if (testResult == null) return null;

            var awvEkgIppeTestResult = (AwvEkgIppeTestResult)testResult;
            if (awvEkgIppeTestResult.ResultImage != null) GetFileDataforResultmedia(awvEkgIppeTestResult.ResultImage);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long orgRoleUserId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.AwvEkgIPPE));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.AwvEkgIPPE, customerId, eventId,
                                          orgRoleUserId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.AwvEkgIPPE, eventCustomerResult.Id);

                var resultMedia = new List<ResultMedia>();
                if (((AwvEkgIppeTestResult)synchronizedTestResult).ResultImage != null)
                    resultMedia.Add(((AwvEkgIppeTestResult)synchronizedTestResult).ResultImage);
        ...
        {
            return GetAllReadings((int)TestType.AwvEkgIPPE);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.AwvEkgIPPE);

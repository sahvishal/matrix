...
{
    public class AwvEkgTestRepository : TestResultRepository
    {
        ...
        public AwvEkgTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public AwvEkgTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new AwvEkgTestResultFactory();
            _testResultSynchronizationService = new AwvEkgTestResultSynchronizationService(callingSource);
            _callingSource = callingSource;
        }

        public AwvEkgTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.AwvEkg);
            var testResult = _testResultFactory.CreateTestResults(customerEventScreeningTests).SingleOrDefault();
            if (testResult == null) return null;

            var awvEkgTestResult = (AwvEkgTestResult)testResult;
            if (awvEkgTestResult.ResultImage != null) GetFileDataforResultmedia(awvEkgTestResult.ResultImage);
            awvEkgTestResult.IsNewResultFlow = isNewResultFlow;
            return testResult;
        }
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long orgRoleUserId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.AwvEkg));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.AwvEkg, customerId, eventId,
                                          orgRoleUserId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.AwvEkg, eventCustomerResult.Id);

                var resultMedia = new List<ResultMedia>();
                if (((AwvEkgTestResult)synchronizedTestResult).ResultImage != null)
                    resultMedia.Add(((AwvEkgTestResult)synchronizedTestResult).ResultImage);
        ...
        {
            return GetAllReadings((int)TestType.AwvEkg);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.AwvEkg);

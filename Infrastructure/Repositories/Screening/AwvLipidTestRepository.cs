...
{
    public class AwvLipidTestRepository : TestResultRepository
    {
        ...
        public AwvLipidTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public AwvLipidTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new AwvLipidTestResultFactory(true);
            _testResultSynchronizationService = new AwvLipidTestResultSynchronizationService(callingSource);
        ...
        public override TestResult GetTestResults(long customerId, long eventId, bool isNewResultFlow)
        {
            ...
            _testResultFactory = new AwvLipidTestResultFactory(customer.Gender == Gender.Female ? false : true);
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.AwvLipid);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long orgRoleUserId)
        {
            ...
            var customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.AwvLipid));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.AwvLipid, customerId, eventId, orgRoleUserId);
                scope.Complete();
                return result;
            }
        }
        ...
        {
            return GetAllReadings((int)TestType.AwvLipid);
        }

        public bool UpdateFraminghamValue(long customerEventScreeningTestId, decimal framinghamValue)
        {
            var testReadingId = GetListOfTestReadingAndReadingId((int)TestType.AwvLipid).Find(testReadingReadingPair =>
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.AwvLipid);

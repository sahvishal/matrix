...
{
    public class PpEchocardiogramTestRepository : TestResultRepository
    {
        ...
        public PpEchocardiogramTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public PpEchocardiogramTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new PpEchocardiogramTestResultFactory();
            _testResultSynchronizationService = new PpEchocardiogramTestResultSyncronizationService(callingSource);
            _callingSource = callingSource;
        }

        public PpEchocardiogramTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            return GetAllReadings((int)TestType.PPEcho);
        }

        public override TestResult GetTestResults(long customerId, long eventId, bool isNewResultFlow)
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.PPEcho);
            var testResult = _testResultFactory.CreateTestResults(customerEventScreeningTests).SingleOrDefault();
            if (testResult == null) return null;

            PpEchocardiogramTestResult echoTestResult = (PpEchocardiogramTestResult)testResult;
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long orgRoleUserId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.PPEcho));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.PPEcho, customerId, eventId,
                                          orgRoleUserId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.PPEcho, eventCustomerResult.Id);
                SaveTestMedia(((PpEchocardiogramTestResult)synchronizedTestResult).Media, customerEventScreeningTestId, synchronizedTestResult.DataRecorderMetaData);
                scope.Complete();
                return result;
            }
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            ...
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.PPEcho);

...
{
    public class SpiroTestRepository : TestResultRepository
    {
        ...
        public SpiroTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public SpiroTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new SpiroTestResultFactory();
            _testResultSynchronizationService = new SpiroTestResultSynchronizationService(callingSource);
            _callingSource = callingSource;
        }

        public SpiroTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.Spiro);

            var testResult = _testResultFactory.CreateTestResults(customerEventScreeningTests).SingleOrDefault();

            if (testResult == null) return null;
            var spiroTestResult = (SpiroTestResult)testResult;
            if (spiroTestResult.ResultImage != null) GetFileDataforResultmedia(spiroTestResult.ResultImage);
            spiroTestResult.IsNewResultFlow = isNewResultFlow;
            return testResult;
        }
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long technicianId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.Spiro));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.Spiro, customerId, eventId, technicianId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.Spiro, eventCustomerResult.Id);

                var resultMedia = new List<ResultMedia>();
                if (((SpiroTestResult)synchronizedTestResult).ResultImage != null)
                    resultMedia.Add(((SpiroTestResult)synchronizedTestResult).ResultImage);
        ...
        {
            return GetAllReadings((int)TestType.Spiro);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.Spiro);

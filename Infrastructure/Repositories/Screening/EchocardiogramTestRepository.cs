...
{
    public class EchocardiogramTestRepository : TestResultRepository
    {
        ...
        public EchocardiogramTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public EchocardiogramTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new EchocardiogramTestResultFactory();
            _testResultSynchronizationService = new EchocardiogramTestResultSyncronizationService(callingSource);
            _callingSource = callingSource;
        }

        public EchocardiogramTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            return GetAllReadings((int)TestType.Echocardiogram);
        }

        public override TestResult GetTestResults(long customerId, long eventId, bool isNewResultFlow)
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.Echocardiogram);
            var testResult = _testResultFactory.CreateTestResults(customerEventScreeningTests).SingleOrDefault();
            if (testResult == null) return null;

            var echoTestResult = (EchocardiogramTestResult)testResult;
            if (echoTestResult.Media != null) GetFileDataforResultmedia(echoTestResult.Media);
            echoTestResult.IsNewResultFlow = isNewResultFlow;
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long orgRoleUserId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.Echocardiogram));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.Echocardiogram, customerId, eventId,
                                          orgRoleUserId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.Echocardiogram, eventCustomerResult.Id);
                SaveTestMedia(((EchocardiogramTestResult)synchronizedTestResult).Media, customerEventScreeningTestId, synchronizedTestResult.DataRecorderMetaData);
                scope.Complete();
                return result;
            }
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                if (existingTestResult != null && existingTestResult.ResultStatus != null && existingTestResult.ResultStatus.StateNumber >= (int)TestResultStateNumber.PreAudit
                    && !(existingTestResult.ResultStatus.StateNumber == (int)TestResultStateNumber.PreAudit && existingTestResult.ResultStatus.Status == TestResultStatus.Incomplete))
                    return true;
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.Echocardiogram);

...
{
    public class MedicareTestRepository : TestResultRepository
    {
        ...
        public MedicareTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public MedicareTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new MedicareTestResultFactory();
            _testResultSynchronizationService = new MedicareTestResultSynchronizationService(callingSource);
            _callingSource = callingSource;
        }

        public MedicareTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.Medicare);

            var testResult = _testResultFactory.CreateTestResults(customerEventScreeningTests).SingleOrDefault();

            if (testResult == null) return null;
            var medicareTestResult = (MedicareTestResult)testResult;
            if (medicareTestResult.ResultImages != null) GetFileDataforResultmedia(medicareTestResult.ResultImages);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long technicianId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.Medicare));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.Medicare, customerId, eventId, technicianId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.Medicare, eventCustomerResult.Id);

                SaveTestMedia(((MedicareTestResult)synchronizedTestResult).ResultImages, customerEventScreeningTestId, synchronizedTestResult.DataRecorderMetaData);
                scope.Complete();
                return result;
            }
        }
        ...
        {
            return GetAllReadings((int)TestType.Medicare);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.Medicare);

...
{
    public class PneumococcalTestRepository : TestResultRepository
    {
        ...
        public PneumococcalTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public PneumococcalTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new PneumococcalTestResultFactory();
            _testResultSynchronizationService = new PneumococcalTestResultSynchronizationService(callingSource);
            _callingSource = callingSource;
        }

        public PneumococcalTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.Pneumococcal);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long technicianId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.Pneumococcal));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.Pneumococcal, customerId, eventId, technicianId);
        ...
        {
            return GetAllReadings((int)TestType.Pneumococcal);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            ...
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.Pneumococcal);

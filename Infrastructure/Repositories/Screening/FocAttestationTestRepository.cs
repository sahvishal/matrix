...
{
    public class FocAttestationTestRepository : TestResultRepository
    {
        ...
        public FocAttestationTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public FocAttestationTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new FocAttestationTestResultFactory();
            _testResultSynchronizationService = new FocAttestationTestResultSynchronizationService(callingSource);
            _callingSource = callingSource;
        }

        public FocAttestationTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.FocAttestation);

            var testResult = _testResultFactory.CreateTestResults(customerEventScreeningTests).SingleOrDefault();

            if (testResult == null) return null;
            var focAttestationTestResult = (FocAttestationTestResult)testResult;
            if (focAttestationTestResult.ResultImage != null) GetFileDataforResultmedia(focAttestationTestResult.ResultImage);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long technicianId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.FocAttestation));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.FocAttestation, customerId, eventId, technicianId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.FocAttestation, eventCustomerResult.Id);

                var resultMedia = new List<ResultMedia>();
                if (((FocAttestationTestResult)synchronizedTestResult).ResultImage != null)
                    resultMedia.Add(((FocAttestationTestResult)synchronizedTestResult).ResultImage);
        ...
        {
            return GetAllReadings((int)TestType.FocAttestation);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.FocAttestation);

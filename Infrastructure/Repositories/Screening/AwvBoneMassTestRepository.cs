...
{
    public class AwvBoneMassTestRepository : TestResultRepository
    {
        ...
        public AwvBoneMassTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public AwvBoneMassTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new AwvBoneMassTestResultFactory();
            _testResultSynchronizationService = new AwvBoneMassTestResultSynchronizationService(callingSource);
            _callingSource = callingSource;
        }

        public AwvBoneMassTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.AwvBoneMass);

            var testResult = _testResultFactory.CreateTestResults(customerEventScreeningTests).SingleOrDefault();
            if (testResult == null) return null;

            var awvBoneMassTestResult = (AwvBoneMassTestResult)testResult;
            if (awvBoneMassTestResult.ResultImage != null) GetFileDataforResultmedia(awvBoneMassTestResult.ResultImage);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long technicianId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity = _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.AwvBoneMass));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.AwvBoneMass, customerId, eventId, technicianId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.AwvBoneMass, eventCustomerResult.Id);

                var resultMedia = new List<ResultMedia>();
                if (((AwvBoneMassTestResult)synchronizedTestResult).ResultImage != null)
                    resultMedia.Add(((AwvBoneMassTestResult)synchronizedTestResult).ResultImage);
        ...
        {
            return GetAllReadings((int)TestType.AwvBoneMass);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.AwvBoneMass);

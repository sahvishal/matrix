...
{
    public class ChlamydiaTestRepository : TestResultRepository
    {
        ...
        public ChlamydiaTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public ChlamydiaTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new ChlamydiaTestResultFactory();
            _testResultSynchronizationService = new ChlamydiaTestResultSynchronizationService(callingSource);
            _callingSource = callingSource;
        }

        public ChlamydiaTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.Chlamydia);

            var testResult = _testResultFactory.CreateTestResults(customerEventScreeningTests).SingleOrDefault();
            if (testResult == null) return null;

            var chlamydiaTestResult = (ChlamydiaTestResult)testResult;
            if (chlamydiaTestResult.ResultImage != null) GetFileDataforResultmedia(chlamydiaTestResult.ResultImage);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long technicianId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.Chlamydia));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.Chlamydia, customerId, eventId, technicianId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.Chlamydia, eventCustomerResult.Id);

                var resultMedia = new List<ResultMedia>();
                if (((ChlamydiaTestResult)synchronizedTestResult).ResultImage != null)
                    resultMedia.Add(((ChlamydiaTestResult)synchronizedTestResult).ResultImage);
        ...
        {
            return GetAllReadings((int)TestType.Chlamydia);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.Chlamydia);

...
{
    public class ThyroidTestRepository : TestResultRepository
    {
        ...
        public ThyroidTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public ThyroidTestRepository(ReadingSource callingSource)
        {
            _settings = new Settings();
            _testResultFactory = new ThyroidTestResultFactory();
            _testResultSynchronizationService = new ThyroidTestResultSynchronizationService(callingSource, _settings.CaptureBloodTest);
            _callingSource = callingSource;
        }

        public ThyroidTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            return GetAllReadings((int)TestType.Thyroid);
        }

        public override TestResult GetTestResults(long customerId, long eventId, bool isNewResultFlow)
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.Thyroid);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long technicianId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.Thyroid));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.Thyroid, customerId, eventId, technicianId);
                scope.Complete();
                return result;
            }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.Thyroid);
        ...
        private bool SaveNewTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)NewTestResultStateNumber.Evaluated)
            {
                ...
                // TODO: need to throw an exception that records are already being audited. Will not be updated.
                //&& !(existingTestResult.ResultStatus.StateNumber == NewTestResultStateNumber.PreAudit && existingTestResult.ResultStatus.Status == TestResultStatus.Incomplete)

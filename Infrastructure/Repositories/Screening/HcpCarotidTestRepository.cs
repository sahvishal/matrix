...
{
    public class HcpCarotidTestRepository : TestResultRepository
    {
        ...
        public HcpCarotidTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public HcpCarotidTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new HcpCarotidTestResultFactory();
            _testResultSynchronizationService = new HcpCarotidTestResultSynchronizationService(callingSource);
            _callingSource = callingSource;
        }

        public HcpCarotidTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.HCPCarotid);

            var testResult = _testResultFactory.CreateTestResults(customerEventScreeningTests).SingleOrDefault();
            if (testResult == null) return null;

            var hcpCarotidTestResult = (HcpCarotidTestResult)testResult;
            if (hcpCarotidTestResult.ResultImages != null)
                GetFileDataforResultmedia(hcpCarotidTestResult.ResultImages);
            hcpCarotidTestResult.IsNewResultFlow = isNewResultFlow;
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long technicianId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.HCPCarotid));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.HCPCarotid, customerId, eventId, technicianId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.HCPCarotid, eventCustomerResult.Id);

                SaveTestMedia(((HcpCarotidTestResult)synchronizedTestResult).ResultImages, customerEventScreeningTestId, synchronizedTestResult.DataRecorderMetaData);
                scope.Complete();
                return result;
            }
        ...
        {
            return GetAllReadings((int)TestType.HCPCarotid);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.HCPCarotid);

...
{
    public class AwvCarotidTestRepository : TestResultRepository
    {
        ...
        public AwvCarotidTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public AwvCarotidTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new AwvCarotidTestResultFactory();
            _testResultSynchronizationService = new AwvCarotidTestResultSynchronizationService(callingSource);
            _callingSource = callingSource;
        }

        public AwvCarotidTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.AwvCarotid);

            var testResult = _testResultFactory.CreateTestResults(customerEventScreeningTests).SingleOrDefault();
            if (testResult == null) return null;

            AwvCarotidTestResult awvCarotidTestResult = (AwvCarotidTestResult)testResult;
            if (awvCarotidTestResult.ResultImages != null)
                GetFileDataforResultmedia(awvCarotidTestResult.ResultImages);
            awvCarotidTestResult.IsNewResultFlow = isNewResultFlow;
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long technicianId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.AwvCarotid));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.AwvCarotid, customerId, eventId, technicianId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.AwvCarotid, eventCustomerResult.Id);

                SaveTestMedia(((AwvCarotidTestResult)synchronizedTestResult).ResultImages, customerEventScreeningTestId, synchronizedTestResult.DataRecorderMetaData);
                scope.Complete();
                return result;
            }
        ...
        {
            return GetAllReadings((int)TestType.AwvCarotid);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.AwvCarotid);

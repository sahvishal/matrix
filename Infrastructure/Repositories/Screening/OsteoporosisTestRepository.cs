...
{
    public class OsteoporosisTestRepository : TestResultRepository
    {
        ...
        public OsteoporosisTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public OsteoporosisTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new OsteoporosisTestResultFactory();
            _testResultSynchronizationService = new OsteoporosisTestResultSynchronizationService(callingSource);
            _callingSource = callingSource;
        }

        public OsteoporosisTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.Osteoporosis);

            var testResult = _testResultFactory.CreateTestResults(customerEventScreeningTests).SingleOrDefault();
            if (testResult == null) return null;

            var osteoporosisTestResult = (OsteoporosisTestResult)testResult;
            if (osteoporosisTestResult.ResultImage != null) GetFileDataforResultmedia(osteoporosisTestResult.ResultImage);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long orgRoleUserId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.Osteoporosis));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.Osteoporosis, customerId, eventId, orgRoleUserId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.Osteoporosis, eventCustomerResult.Id);

                var resultMedia = new List<ResultMedia>();
                if (((OsteoporosisTestResult)synchronizedTestResult).ResultImage != null)
                    resultMedia.Add(((OsteoporosisTestResult)synchronizedTestResult).ResultImage);
        ...
        {
            return GetAllReadings((int)TestType.Osteoporosis);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.Osteoporosis);

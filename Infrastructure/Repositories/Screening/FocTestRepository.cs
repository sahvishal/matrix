...
{
    public class FocTestRepository : TestResultRepository
    {
        ...
        public FocTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public FocTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new FocTestResultFactory(true);
            _testResultSynchronizationService = new FocTestResultSynchronizationService(callingSource);
        ...
        public override TestResult GetTestResults(long customerId, long eventId, bool isNewResultFlow)
        {
            ...
            if (focTestResult.ResultImage != null) GetFileDataforResultmedia(focTestResult.ResultImage);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long orgRoleUserId)
        {
            ...
            var customerEventScreeningEntity = _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.Foc));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.Foc, customerId, eventId, orgRoleUserId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.Foc, eventCustomerResult.Id);


                var resultMedia = new List<ResultMedia>();
                if (((FocTestResult)synchronizedTestResult).ResultImage != null)
                    resultMedia.Add(((FocTestResult)synchronizedTestResult).ResultImage);
        ...
        {
            return GetAllReadings((int)TestType.Foc);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.Foc);

...
{
    public class PulmonaryFunctionTestRepository : TestResultRepository
    {
        ...
        public PulmonaryFunctionTestRepository()
        ...
        public PulmonaryFunctionTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            return GetAllReadings((int)TestType.PulmonaryFunction);
        }
        ...
        public override TestResult GetTestResults(long customerId, long eventId, bool isNewResultFlow)
        {
            ...
            if (pulmonaryTestResult.ResultImage != null) GetFileDataforResultmedia(pulmonaryTestResult.ResultImage);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long technicianId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.PulmonaryFunction));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.PulmonaryFunction, customerId, eventId, technicianId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.PulmonaryFunction, eventCustomerResult.Id);

                var resultMedia = new List<ResultMedia>();
                if (((PulmonaryFunctionTestResult)synchronizedTestResult).ResultImage != null)
                    resultMedia.Add(((PulmonaryFunctionTestResult)synchronizedTestResult).ResultImage);
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId,ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.PulmonaryFunction);

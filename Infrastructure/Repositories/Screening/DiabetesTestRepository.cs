...
{
    public class DiabetesTestRepository : TestResultRepository
    {
        ...
        public DiabetesTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public DiabetesTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new DiabetesTestResultFactory(true);
            _testResultSynchronizationService = new DiabetesTestResultSynchronizationService(callingSource);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long orgRoleUserId)
        {
            ...
            var customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.Diabetes));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.Diabetes, customerId, eventId, orgRoleUserId);
                scope.Complete();
                return result;
            }
        }
        ...
        {
            return GetAllReadings((int)TestType.Diabetes);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            ...
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.Diabetes);
        ...
        private bool SaveNewTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            ...
            else
            {
                ...
                else if (currentTestResult.ResultStatus.StateNumber < (int)NewTestResultStateNumber.Evaluated)
                {
                    TestResult existingTestResult = null;
                    lock (CentralLocker.Locker)
                    {

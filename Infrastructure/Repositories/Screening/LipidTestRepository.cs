...
{
    public class LipidTestRepository : TestResultRepository
    {
        ...
        public LipidTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public LipidTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new LipidTestResultFactory(true);
            _testResultSynchronizationService = new LipidTestResultSynchronizationService(callingSource);
        ...
        public override TestResult GetTestResults(long customerId, long eventId, bool isNewResultFlow)
        {
            ...
            _testResultFactory = new LipidTestResultFactory(customer.Gender == Gender.Female ? false : true);
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.Lipid);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long orgRoleUserId)
        {
            ...
            var customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.Lipid));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.Lipid, customerId, eventId, orgRoleUserId);
                scope.Complete();
                return result;
            }
        }
        ...
        {
            return GetAllReadings((int)TestType.Lipid);
        }

        public bool UpdateFraminghamValue(long customerEventScreeningTestId, decimal framinghamValue)
        {
            var testReadingId = GetListOfTestReadingAndReadingId((int)TestType.Lipid).Find(testReadingReadingPair =>
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId,ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.Lipid);

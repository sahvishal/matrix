...
{
    public class HcpEchocardiogramTestRepository : TestResultRepository
    {
        ...
        public HcpEchocardiogramTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public HcpEchocardiogramTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new HcpEchocardiogramTestResultFactory();
            _testResultSynchronizationService = new HcpEchocardiogramTestResultSyncronizationService(callingSource);
            _callingSource = callingSource;
        }

        public HcpEchocardiogramTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            return GetAllReadings((int)TestType.HCPEcho);
        }
        ...
        public override TestResult GetTestResults(long customerId, long eventId, bool isNewResultFlow)
        {
            ...
            if (hcpEchoTestResult.Media != null) GetFileDataforResultmedia(hcpEchoTestResult.Media);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long orgRoleUserId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.HCPEcho));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.HCPEcho, customerId, eventId,
                    orgRoleUserId);
                ...
                var customerEventScreeningTestId = GetCustomerEventScreeningTestId((int)TestType.HCPEcho, eventCustomerResult.Id);
                SaveTestMedia(((HcpEchocardiogramTestResult)synchronizedTestResult).Media, customerEventScreeningTestId, synchronizedTestResult.DataRecorderMetaData);
                scope.Complete();
                return result;
            }
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.HCPEcho);

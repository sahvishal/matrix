...
{
    public class HPyloriTestRepository : TestResultRepository
    {
        ...
        public HPyloriTestRepository()
            : this(ReadingSource.Manual)
        {
        }

        public HPyloriTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new HPyloriTestResultFactory();
            _testResultSynchronizationService = new HPyloriTestResultSynchronizationService(callingSource);
            _callingSource = callingSource;
        }

        public HPyloriTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.HPylori);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long technicianId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.HPylori));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.HPylori, customerId, eventId, technicianId);
        ...
        {
            return GetAllReadings((int)TestType.HPylori);
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.HPylori);

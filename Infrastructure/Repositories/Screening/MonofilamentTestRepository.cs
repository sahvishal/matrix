...
{
    public class MonofilamentTestRepository : TestResultRepository
    {
        ...
        public MonofilamentTestRepository()
            : this(ReadingSource.Manual)
        {

        }

        public MonofilamentTestRepository(ReadingSource callingSource)
        {
            _testResultFactory = new MonofilamentTestResultFactory();
            _testResultSynchronizationService = new MonofilamentTestResultSynchronizationService(callingSource);
            _callingSource = callingSource;
        }

        public MonofilamentTestRepository(IPersistenceLayer persistenceLayer, ITestResultFactory testResultFactory, ITestResultSynchronizationService testResultSynchronizationService)
        ...
        {
            return GetAllReadings((int)TestType.Monofilament);
        }

        public override TestResult GetTestResults(long customerId, long eventId, bool isNewResultFlow)
        {
            List<CustomerEventScreeningTestsEntity> customerEventScreeningTests = GetTestResultsByTestId(customerId, eventId, (int)TestType.Monofilament);
        ...
        public override bool SaveTestResults(TestResult currentTestResult, long customerId, long eventId, long technicianId)
        {
            ...
            CustomerEventScreeningTestsEntity customerEventScreeningEntity =
                _testResultFactory.CreateTestResultEntity(synchronizedTestResult, GetListOfTestReadingAndReadingId((int)TestType.Monofilament));

            using (var scope = new TransactionScope())
            {
                var result = PersistTestResults(customerEventScreeningEntity, (int)TestType.Monofilament, customerId, eventId, technicianId);
                scope.Complete();
                return result;
            }
        }
        ...
        private bool SaveOldTestResult(TestResult currentTestResult, long customerId, long eventId, ref TestResult synchronizedTestResult)
        {
            ...
            if (currentTestResult != null && currentTestResult.ResultStatus != null && currentTestResult.ResultStatus.StateNumber < (int)TestResultStateNumber.Evaluated)
            {
                ...
                var compareToResultReadings = GetAllReadings((int)TestType.Monofilament);

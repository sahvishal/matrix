...
namespace Falcon.App.Infrastructure.Finance.Factories
{
    ...
    public class GiftCertificateWellmedReportFactory : IGiftCertificateWellmedReportFactory
    {
        public ListModelBase<GiftCertificateReportWellmedViewModel, GiftCertificateReportFilter> Create(IEnumerable<Customer> customers, IEnumerable<EventCustomer> eventCustomers, EventVolumeListModel eventListModel, IEnumerable<Order> orders,
           IEnumerable<OrderedPair<long, string>> packages, IEnumerable<OrderedPair<long, string>> tests)
        {
            var model = new GiftCertificateReportListWellmedModel();
            var customerExportModels = new List<GiftCertificateReportWellmedViewModel>();
            ...
            foreach (var eventCustomer in eventCustomers)
            {
                ...
                if (customer != null)
                {
                    var customerExportModel = new GiftCertificateReportWellmedViewModel
                    {

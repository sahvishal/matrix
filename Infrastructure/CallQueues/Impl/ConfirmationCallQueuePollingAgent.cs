...
namespace Falcon.App.Infrastructure.CallQueues.Impl
{
    ...
    public class ConfirmationCallQueuePollingAgent : IConfirmationCallQueuePollingAgent
    {
        ...
        public void PollForCallQueue()
        {
            try
            {
                _logger.Info("Starting single call queue customer for confirmation");
                var callQueue = _callQueueRepository.GetCallQueueByCategory(CallQueueCategory.Confirmation);
                ...
                foreach (var criteria in criterias)
                {
                    ...
                    var callQueueCustomers = _confirmationCallQueueService.GetCallQueueCustomers(callQueue.Id, criteria.NoOfDays);
            ...
            {
                _logger.Error(string.Format("Error while pulling single Confirmation  Event call queue. Message {0} \n Stack Trace {1}", ex.Message, ex.StackTrace));
            }
        }
    }
}

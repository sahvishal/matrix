...
select cqc.CallQueueCustomerId	
  ...
  ,ISNULL(cp.ProductTypeId, 0) ProductTypeId
     from TblCallQueueCustomer cqc with(nolock)
		...
		INNER JOIN TblCustomerProfile cp WITH (NOLOCK) ON cp.CustomerID = cqc.CustomerId
		--AND cqc.CallDate > CAST(CONVERT(VARCHAR, DATEPART(yyyy,GETDATE())) + '-1-1' AS DATETIME)
	...
	( 
		exists
			(
				...
				where tt.ProductTypeId = cp.ProductTypeId
			)
		or exists 
				(

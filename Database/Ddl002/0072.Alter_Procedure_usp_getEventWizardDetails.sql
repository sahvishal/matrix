...
BEGIN      
 ...
 SELECT TE.EventID      
   ...
   ,TE.IsManual      
   ,TE.UpdatedByAdmin ,    
   TE.AllowNonMammoPatients,
   ProductType=STUFF((
            SELECT ',' + convert(nvarchar(5),ProductTypeId)
            FROM TblEventProductType  where EventID=@EventID And IsActive=1 And EndDate is null
            FOR XML PATH('')
            ), 1, 1, '')
 ...
 TE.EventID      
   ...
   ,TEPD.Gender      
   ,TEPD.IsRecommended       ,TEPD.MostPopular      
   ,TEPD.BestValue      
   ,TEPD.PodRoomID      
 FROM dbo.TblEvents TE      

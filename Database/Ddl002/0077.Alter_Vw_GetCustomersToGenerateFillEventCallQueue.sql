...
ALTER VIEW [dbo].[Vw_GetCustomersToGenerateFillEventCallQueue]   
As 
 ...
 SELECT CP.CustomerID    
  ...
  FROM TblCustomerProfile AS CP WITH (NOLOCK) INNER JOIN  
	   ...
	   Left Outer join   
       (  
			SELECT CustomerId, IsTargated FROM TblCustomerTargeted WITH (NOLOCK) WHERE ForYear = DATEPART(YEAR, GETDATE())  
       ) CT on CP.CustomerID = CT.CustomerId  
    ...
    WHERE   
	 ...
	 AND CP.ProductTypeId IS NOT NULL AND CP.ProductTypeId <> ''

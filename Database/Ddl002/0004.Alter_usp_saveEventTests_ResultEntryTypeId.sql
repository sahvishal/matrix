...
BEGIN
	...
	DECLARE @EventDate DATETIME, @IsHealthPlan BIT, @IsRecordable BIT
	...
	SELECT @RefundPrice = RefundPrice, @HafTemplateId = HAFTemplateId, @Gender = Gender, @IsRecordable = IsRecordable FROM TblTest WHERE TestId = @TestId

	SET @ResultEntryTypeId = CASE WHEN @IsRecordable = 0 THEN NULL 
	WHEN @IsHealthPlan = 0 THEN @HipQuestionnaire 
	WHEN @IsHealthPlan = 1 THEN @ChatQuestionnaire 

...
inner join 
	(
		Select A.EventCustomerResultId, A.EventId, A.CustomerId, B.PhysicianId, B.OverReadPhysicianId, ResultState, ResultSummary, isnull(InQueuePriority, 0) InQueuePriority 
			from TblEventCustomerResult A with(nolock) inner join TblPhysicianEventAssignment B with(nolock)
				on A.EventId = B.EventId
		where ((A.ResultState = 4 and A.IsPartial = 0) or (A.ResultState = 5 and A.IsPartial = 1)) and B.IsActive = 1
				and EventCustomerResultId not in (select EventCustomerId from TblPhysicianCustomerAssignment with(nolock) where IsActive = 1)
		union		
		Select A.EventCustomerResultId, A.EventId, A.CustomerId, B.PhysicianId, B.OverReadPhysicianId, ResultState, ResultSummary, isnull(InQueuePriority, 0) InQueuePriority 
		from TblEventCustomerResult A with(nolock) inner join TblPhysicianCustomerAssignment B with(nolock)

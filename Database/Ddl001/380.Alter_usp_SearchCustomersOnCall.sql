...
--  END  
  ...
  set @sel_CorporateCustomerTag= 'Select CustomerId,Tag from TblCustomerTag where CustomerId in (SELECT DISTINCT TOP 21 isnull(TC.CustomerID,0)CustomerID 
     from TblUser TU 
     inner join TblAddress TA on TA.AddressID = TU.HomeAddressID   
     inner join TblUserLogin TUL on TU.UserID=TUL.UserLoginID  
     left outer join vw_Customers TC on  TU.UserID = TC.UserID        
...
--  END  
  ...
  Set @sel_CorporateCustomerTag1 ='Select CustomerId,Tag from TblCustomerTag where CustomerId in (SELECT DISTINCT TOP 21 isnull(TC.CustomerID,0)CustomerID
     from TblUser TU 
     inner join TblAddress TA on TA.AddressID = TU.HomeAddressID   
     inner join TblUserLogin TUL on TU.UserID=TUL.UserLoginID  
     left outer join vw_Customers TC on TU.UserID = TC.UserID      

...
/****** Object:  View [dbo].[Vw_GetCustomersForCallQueueWithCustomer]    Script Date: 2/26/2018 5:47:25 AM ******/
SET ANSI_NULLS ON
GO
...
select cqc.CallQueueCustomerId	
  ...
  ,IsNull(cqc.IncorrectPhoneNumberMarkedDate,'01/01/1900') as IncorrectPhoneNumberMarkedDate
	...
	(
		...
		(
			SELECT Tag FROM TblCorporateTag WITH(NOLOCK) WHERE StartDate >= '11/01/2017' and EndDate >= CONVERT(DATE, GETDATE())
		)
	)

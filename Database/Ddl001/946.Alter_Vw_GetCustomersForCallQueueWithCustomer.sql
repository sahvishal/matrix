...
/****** Object:  View [dbo].[Vw_GetCustomersForCallQueueWithCustomer]    Script Date: 9/12/2018 10:18:17 AM ******/
SET ANSI_NULLS ON
GO
...
select cqc.CallQueueCustomerId	
  ...
  ,IsNull(cqc.NoShowDate,cast('01/01/1900' as datetime)) as NoShowDate
     ...
     from TblCallQueueCustomer cqc with(nolock)
		INNER JOIN  TblHealthPlanCallQueueCriteriaAssignment AS hpcqca WITH (NOLOCK) ON cqc.CallQueueCustomerId = hpcqca.CallQueueCustomerId AND cqc.CallDate > CAST(CONVERT(VARCHAR, DATEPART(yyyy,GETDATE())) + '-1-1' AS DATETIME)

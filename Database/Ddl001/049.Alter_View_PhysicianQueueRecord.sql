...
Select Q.EventCustomerResultId, EventId, CustomerId, PhysicianId, OverreadPhysicianId, Convert(bit, (Case When ResultState = 5 then 1 else 0 end)) IsAtOverreadState,  Z.EvaluatedByOrgRoleUserId,
	...
	InQueuePriority, isnull(Z.UpdatedOn, Z.CreatedOn) as UpdatedOn from 
	(
		Select A.EventCustomerResultId, A.EventId, A.CustomerId, B.PhysicianId, B.OverReadPhysicianId, ResultState, ResultSummary, isnull(InQueuePriority, 0) InQueuePriority from TblEventCustomerResult A inner join TblPhysicianEventAssignment B
		...
		union		
		Select A.EventCustomerResultId, A.EventId, A.CustomerId, B.PhysicianId, B.OverReadPhysicianId, ResultState, ResultSummary, isnull(InQueuePriority, 0) InQueuePriority from TblEventCustomerResult A inner join TblPhysicianCustomerAssignment B
...
where Q.EventCustomerResultId not in (Select EventCustomerId from TblCustomerSkipReview where IsActive = 1)

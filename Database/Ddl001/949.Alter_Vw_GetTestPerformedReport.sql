...
/****** Object:  View [dbo].[Vw_GetTestPerformedReport]    Script Date: 9/18/2018 9:23:09 AM ******/
SET ANSI_NULLS ON
GO
...
As
	...
	SELECT      ec.EventId, ec.CustomerId, e.EventDate, p.Name AS Pod, accountTemp.AccountId, accountTemp.IsHealthPlan, 
		...
		OUTER APPLY(
			SELECT top 1 * FROM TblEventCustomerResult ecr WITH (nolock) WHERE ecr.CustomerID = ec.CustomerID AND ecr.EventID = ec.EventID ORDER BY ecr.EventCustomerResultId DESC
		) EventCustomerResult

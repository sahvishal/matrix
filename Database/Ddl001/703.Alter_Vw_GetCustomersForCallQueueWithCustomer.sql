...
/****** Object:  View [dbo].[Vw_GetCustomersForCallQueueWithCustomer]    Script Date: 05-01-2018 20:04:14 ******/
SET ANSI_NULLS ON
GO
...
select cqc.CallQueueCustomerId	
  ...
  ,IsNull(cqc.AppointmentCancellationDate,'01/01/1900') as AppointmentCancellationDate
     ...
     from TblCallQueueCustomer cqc with(nolock)
		...
		And (cqc.IsIncorrectPhoneNumber =  0 )
		AND cqc.[Status] = 163 		
		AND (
		...
		AND	
		(	
			(cqc.ContactedDate is null  or	cqc.ContactedDate < CONVERT(date,GETDATE()) )
			OR 
			(

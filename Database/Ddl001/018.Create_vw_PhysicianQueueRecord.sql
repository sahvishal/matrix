...
Create View vw_PhysicianQueueRecord
As
Select Q.EventCustomerResultId, EventId, CustomerId, PhysicianId, OverreadPhysicianId, Convert(bit, (Case When ResultState = 5 then 1 else 0 end)) IsAtOverreadState,  Z.EvaluatedByOrgRoleUserId,
	Convert(bit, Z.SelfPresent) CriticalMarkedByTechnician, ResultSummary, isnull(Z.UpdatedOn, Z.CreatedOn) as UpdatedOn from (
...
inner join 
	(Select EventCustomerResultId, Max(EvaluatedByOrgRoleUserId) EvaluatedByOrgRoleUserId, Max(CreatedOn) CreatedOn, Max(UpdatedOn) UpdatedOn, Max(Convert(int, SelfPresent)) SelfPresent from TblCustomerEventScreeningTests X inner join TblCustomerEventTestState Y on X.CustomerEventScreeningTestId = Y.CustomerEventScreeningTestId 
		where evaluationstate between 4 and 5 
	group by X.EventCustomerResultId) Z

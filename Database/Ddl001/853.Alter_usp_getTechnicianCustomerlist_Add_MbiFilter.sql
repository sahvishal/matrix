USE [$(dbname)]
GO
/****** Object:  StoredProcedure [dbo].[usp_getTechnicianCustomerlist]    Script Date: 2/28/2018 6:36:51 PM ******/
SET ANSI_NULLS ON
GO
...
BEGIN
	  ...
	  Declare @EventId_Parameter bigint
	  IF(@eventId is NOT NULL)
	  BEGIN
		SET @EventId_Parameter = CAST(@eventId as bigint)
	  END
	  ELSE
	  BEGIN
		SET @EventId_Parameter = NULL
	  END
				...
				BEGIN
					...
					(
						SELECT  TC.CustomerID,
							...
							inner join vw_Customers TC WITH (NOLOCK) on TU.UserId = TC.UserId 								
								...
								and (
										...
										(
											...
											(
												select AccountId from TblAccountCallCenterOrganization where IsDeleted = 0 and OrganizationId = @OrganizationId
											)
										)
									)
							...
							--left outer join 
							--(
							--	select CustomerId, max(TEC.EventCustomerId) EventCustomerId,TE.EventDate,TE.EventName,TE.EventID from TblEventCustomers TEC WITH (NOLOCK)
							...
							--left outer join
							--(
							--	select TE.EventId, TE.EventName, TE.EventDate, TEC.EventCustomerId
							...
							--WHERE TU.IsActive=1 and ((EventCust.EventCustomerId>0 and RegData.EventCustomerId>0) or (EventCust.EventCustomerId is null and RegData.EventCustomerId is null and @EventId_Parameter is null and @datefrom is null and @dateto is null))
							WHERE TU.IsActive=1 --and ((EventCust.EventCustomerId > 0 ) or (EventCust.EventCustomerId is null and @EventId_Parameter is null and @datefrom is null and @dateto is null))
				...
				BEGIN
					...
					(
						SELECT  TC.CustomerID,
							...
							inner join vw_Customers TC WITH (NOLOCK) on TU.UserId = TC.UserId 
								and 1 = case when @FirstName is not null then (case when TU.Firstname like  isnull(@FirstName, '') + '%' then 1 else 0 end) else 1 end
								--and TU.Firstname like  isnull(@FirstName, '') + '%' 

								and 1 = case when @LastName is not null then (case when TU.LastName like  isnull(@LastName, '') + '%' then 1 else 0 end) else 1 end
								...
								and 1 = case when @PhoneNumber is not null then (case when TU.PhoneHome = @PhoneNumber or TU.PhoneCell=@PhoneNumber or TU.PhoneOffice=@PhoneNumber then 1 else 0 end) else 1 end
								and 1 = case when @Email is not null then (case when TU.Email1 like '%'+ @Email + '%' then 1 else 0 end) else 1 end

								and 1 = case when @MemberId is not null then (case when TC.InsuranceId=@MemberId then 1 else 0 end) else 1 end
								--and (@MemberId is null or TC.InsuranceId=@MemberId  ) 
								and 1 = case when @HICN is not null then (case when TC.HICN=@HICN then 1 else 0 end) else 1 end 
								and 1 = case when @Mbi is not null then (case when TC.Mbi = @Mbi then 1 else 0 end) else 1 end 
								--and (@HICN is null or TC.HICN=@HICN )
								and 1 = case when @dob is not null then (case when TU.DOB=@dob then 1 else 0 end) else 1 end

							--left outer join 
							--(
							--	select CustomerId, max(TEC.EventCustomerId) EventCustomerId,TE.EventId, TE.EventName, TE.EventDate from TblEventCustomers TEC WITH (NOLOCK)
							--	INNER JOIN TblEventCustomerOrderDetail TECOD WITH (NOLOCK) on TECOD.EventCustomerId = TEC.EventCustomerID 
								
							--	and 1 = case when @EventId_Parameter is not null then (case when TEC.EventId = @EventId_Parameter then 1 else 0 end) else 1 end 								
							--	inner join TblEvents TE WITH (NOLOCK) on TEC.EventID = TE.EventID 
...
--                END
				  ...
				  IF(@dob is not null)
				  BEGIN
					SET @sel_events_query = @sel_events_query + ' AND TU.DOB = CAST(''' + CONVERT(VARCHAR,@dob,101) + '''AS DATETIME)'
				  END
...
--                END
				  ...
				  IF(@dob is not null)
				  BEGIN
					SET @sel_address_query = @sel_address_query + ' AND TU.DOB = CAST(''' + CONVERT(VARCHAR,@dob,101) + '''AS DATETIME)'
				  END

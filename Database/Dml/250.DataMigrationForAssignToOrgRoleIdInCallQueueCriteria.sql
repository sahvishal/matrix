		...
		Begin
			Begin Try
				Begin Tran
					...
					Begin
						Declare @Percentage decimal(18,2),@NoOfDays int

						Select @Percentage=Percentage,@NoOfDays=NoOfDays from TblHealthPlanCallQueueCriteria where Id = @criteriaId and IsDeleted = 0 and IsDefault = 0		

						Insert Into TblHealthPlanCriteriaAssignment (HealthPlanCriteriaId,AssignedToOrgRoleUserId)
						select @criteriaId,AssignedToOrgRoleUserId  from TblHealthPlanCallQueueCriteria  where Percentage=@Percentage and NoOfDays=@NoOfDays And HealthPlanId=@HealthPlanId and CallQueueId=@CallQueueId 		
						...
						Update TblHealthPlanCallQueueCriteria Set IsDeleted=1 where Percentage=@Percentage and NoOfDays=@NoOfDays And HealthPlanId=@HealthPlanId and CallQueueId=@CallQueueId and Id <> @criteriaId

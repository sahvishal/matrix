...
select @CallQueueId = CallQueueId  from TblCallQueue where Category='ZipRadius'
			...
			Begin Try
				Begin Tran
					...
					Begin
						...
						Declare @Radius int,@Zip varchar(55)

						Select @Radius=Radius,@Zip=ZipCode from TblHealthPlanCallQueueCriteria where Id = @criteriaId and IsDeleted = 0 and IsDefault = 0
				
						Insert Into TblHealthPlanCriteriaAssignment (HealthPlanCriteriaId,AssignedToOrgRoleUserId)
						select @criteriaId,AssignedToOrgRoleUserId  from TblHealthPlanCallQueueCriteria  where Radius = @Radius and ZipCode = @Zip And HealthPlanId = @HealthPlanId and CallQueueId = @CallQueueId 		
						...
						Update TblHealthPlanCallQueueCriteria Set IsDeleted = 1 where Radius=@Radius and ZipCode=@Zip And HealthPlanId=@HealthPlanId and CallQueueId=@CallQueueId and Id <> @criteriaId

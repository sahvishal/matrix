...
INSERT INTO #TempTable 
	(criteriaId,CallQueueId,HealthPlanId)
...
select @MaxRow = Max(rowcnt) from #TempTable
	...
	Begin 				
		...
		Begin
			Begin Try
				Begin Tran
					Declare @HealthPlanId BigInt,@criteriaId BigInt,@IsAlreadyAdded bit 		
					...
					Begin
						...
						Declare @RoundOfCalls int,@NoOfDays int

						Select @RoundOfCalls=RoundOfCalls,@NoOfDays=NoOfDays from TblHealthPlanCallQueueCriteria where Id=@criteriaId and IsDeleted=0 and IsDefault=0
														
						Insert Into TblHealthPlanCriteriaAssignment (HealthPlanCriteriaId,AssignedToOrgRoleUserId)
						select @criteriaId,AssignedToOrgRoleUserId  from TblHealthPlanCallQueueCriteria  where RoundOfCalls=@RoundOfCalls and NoOfDays=@NoOfDays And HealthPlanId=@HealthPlanId and CallQueueId=@CallQueueId 		
						...
						Update TblHealthPlanCallQueueCriteria Set IsDeleted=1 where RoundOfCalls=@RoundOfCalls and NoOfDays=@NoOfDays And HealthPlanId=@HealthPlanId and CallQueueId=@CallQueueId and Id <> @criteriaId

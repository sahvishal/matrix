...
namespace Falcon.App.Core.Application.Helper
{
    public static class DirectoryOperationsHelper
    {
        ...
        public static void CopyWithReplace(string sourceFileName, string destFileName)
        {
            if (VaidateFullPath(sourceFileName) && VaidateFullPath(destFileName))
            {
                DeleteFileIfExist(destFileName);
                File.Copy(sourceFileName, destFileName);
            }
        }
        ...
        {
            return VaidateFullPath(filePath) ? new FileInfo(filePath).Length : 0;
        }
        ...
        public static void MoveFileWithReplace(string sourceFileName, string destinationPath)
        {
            if (VaidateFullPath(sourceFileName) && VaidateFullPath(destinationPath))
            {
                if (IsFileExist(sourceFileName))
                {
                    DeleteFileIfExist(destinationPath);
